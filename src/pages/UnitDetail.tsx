...
import {
  MapPin, CheckCircle, ArrowLeft, Factory, Cog, Phone, Mail,
  Wrench, Globe2, Send,
...
const UnitDetail = () => {
  ...
  return (
    ...
    >
      ...
      <section ref={heroRef} className="relative pt-36 pb-28 bg-gradient-dark overflow-hidden">
        ...
        <div className="container relative">
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            <Link to="/units" className="inline-flex items-center gap-1 text-sm text-steel hover:text-primary-foreground transition-colors mb-6">
      ...
      {/* ═══ 3. KEY CAPABILITIES as icon cards ═══ */}
      <section className="py-20 bg-muted/50">
        <div className="container">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
            {unit.capabilities.map((cap) => {
              const Icon = capabilityIcons[cap] || CheckCircle;
              return (
                <motion.div key={cap} variants={scaleUp}
                  className="bg-background rounded-xl p-5 shadow-card border border-border/50 transition-all duration-300 text-center group"
                  whileHover={{ y: -4, transition: { duration: 0.3 } }}>
                  <div className="w-11 h-11 rounded-lg bg-gradient-industrial flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform duration-300 shadow-industrial">
                    <Icon className="h-5 w-5 text-primary-foreground" />
                  </div>
                  <p className="text-sm font-medium text-foreground leading-tight">{cap}</p>
                </motion.div>
              );
            })}
          </motion.div>
      ...
      {/* ═══ 4. MACHINES IN THIS UNIT ═══ */}
      <section className="py-20 bg-background">
        <div className="container">
          <SectionHeading label="Machinery" title="Machines in this Unit" description={`${unit.machines.length} precision machines installed at ${unit.location}.`} />
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto">
            {unit.machines.map((m, i) => (
              <motion.div key={`${m.brand}-${m.model}-${i}`} variants={scaleUp}
                ...
                <div className="p-5">
                  <p className="font-heading font-bold text-foreground text-base">{m.name}</p>
