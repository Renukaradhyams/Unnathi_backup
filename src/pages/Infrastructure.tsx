...
import { machineryPhotos, infrastructurePhotos, qualityLabPhotos } from "@config/units.config";
...
const machineImgMap: Record<string, string> = {
  "CNC Machining Centers": machineryPhotos[1].src,
...
/* ── gallery images: 7.1 Factory + 7.2 Machinery + 7.3 Infrastructure + 7.4 Quality Lab ── */
const galleryImages = [
  ...machineryPhotos.map((p) => p.src),
  ...infrastructurePhotos.map((p) => p.src),
  ...qualityLabPhotos.map((p) => p.src),
  pageSectionMedia.infrastructure.sections.galleryExtra.asset.src,
];
...
const certifications = [
  ...
  { name: "ZED Silver Certification", file: pageSectionMedia.infrastructure.sections.certificateZed.asset.src, preview: pageSectionMedia.infrastructure.sections.certificateZed.asset.src, fileType: "image", desc: "Zero Defect Zero Effect manufacturing" },
];
...
const Infrastructure = () => {
  ...
  return (
    <Layout seo={buildPageSeo("infrastructure", [{ name: "Home", path: "/" }, { name: "Infrastructure", path: "/infrastructure" }], undefined, seoImages.filter((i) => i.path === "/infrastructure").map((i) => ({ name: i.title, imageUrl: i.image, caption: i.caption, pagePath: i.path, width: i.width, height: i.height, encodingFormat: i.mimeType })))}>
      ...
      <section className="relative -mt-16 z-10">
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}
            ...
            {[
              { end: 12, suffix: "+", label: "Years in Industry" },
        ...
        <div className="container grid md:grid-cols-2 gap-16 items-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={slideLeft}>
            ...
            <p className="mt-5 text-muted-foreground leading-relaxed">
              Our state-of-the-art manufacturing facility is equipped with advanced CNC machining centers, turning centers, and precision grinding machines. The shop floor operates under stringent quality controls with a focus on efficiency, safety, and precision.
            </p>
      ...
      <section ref={machineryRef} className="py-24 bg-muted/30 overflow-hidden scroll-mt-24">
        <div className="container">
            ...
            <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 pointer-events-none">
              ...
              >
                <div className="relative">
                  <img src={selectedMetro.images[0] || metroImg} alt={`${selectedMetro.brand} ${selectedMetro.model}`} className="w-full h-56 object-cover rounded-t-2xl" />
                  <button onClick={() => setSelectedMetro(null)} className="absolute top-3 right-3 p-2 rounded-full bg-charcoal/70 text-primary-foreground hover:bg-charcoal transition-colors" aria-label="Close">
                    <X className="h-5 w-5" />
                  </button>
                </div>
          ...
          <motion.div className="grid grid-cols-2 md:grid-cols-3 gap-4" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            {galleryImages.map((img, i) => (
              <motion.div key={i} variants={scaleUp}
                ...
                whileHover={{ y: -4, transition: { duration: 0.3 } }}
                onClick={() => setLightbox({ images: galleryImages.map((src, idx) => ({ src, label: galleryLabels[idx] || `View ${idx + 1}` })), index: i })}>
                <motion.img
                  src={img}
                  alt={galleryLabels[i] || `Facility view ${i + 1}`}
                  className="w-full h-full object-cover"
                  whileHover={{ scale: 1.08 }}
                  transition={{ duration: 0.6 }}
                  loading="lazy"
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-charcoal/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                  <p className="text-sm font-heading font-semibold text-primary-foreground">{galleryLabels[i]}</p>
                </div>
              </motion.div>
            ))}
          </motion.div>
